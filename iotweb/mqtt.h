...
enum MqttError {
    ...
    MQTTERR_ILLEGAL_CHARACTER        = -5, /**< 非法的字窿 */
    MQTTERR_NOT_UTF8                 = -6, /**< 字符编码不是UTF-8 */
    MQTTERR_INVALID_PARAMETER        = -7, /**< 参数错误 */
    MQTTERR_PKT_TOO_LARGE            = -8, /**< 数据包过墿 */
    MQTTERR_BUF_OVERFLOW             = -9, /**< 缓冲区溢兿 */
    ...
    MQTTERR_INCOMPLETE_SUBOBJECT     = -13,/**< 调用Mqtt_PackDataPointFinish时，包含的子数据结构不完擿 */
    MQTTERR_FAILED_SEND_RESPONSE     = -14 /**< 处理publish系列消息后，发翁响应包失败 */
};

/** MQTT数据包类圿 */
enum MqttPacketType {
    MQTT_PKT_CONNECT = 1, /**< 连接请求数据势 */
    MQTT_PKT_CONNACK,     /**< 连接确认数据势 */
    MQTT_PKT_PUBLISH,     /**< 发布数据数据势 */
    MQTT_PKT_PUBACK,      /**< 发布确认数据势 */
    ...
    MQTT_PKT_SUBSCRIBE,   /**< 订阅数据势 */
    MQTT_PKT_SUBACK,      /**< 订阅确认数据势 */
    MQTT_PKT_UNSUBSCRIBE, /**< 取消订阅数据势 */
    MQTT_PKT_UNSUBACK,    /**< 取消订阅确认数据势 */
    MQTT_PKT_PINGREQ,     /**< ping 数据势 */
    MQTT_PKT_PINGRESP,    /**< ping 响应数据势 */
    MQTT_PKT_DISCONNECT   /**< 断开连接数据势 */
};

/** MQTT QOS等级 */
enum MqttQosLevel {
    MQTT_QOS_LEVEL0,  /**< 暿多发送一檿 */
    MQTT_QOS_LEVEL1,  /**< 暿少发送一檿  */
    MQTT_QOS_LEVEL2   /**< 只发送一檿 */
};
...
enum MqttRetCode {
    ...
    MQTT_CONNACK_BAD_USER_NAME_OR_PASSWORD = 4, /**< 用户名或密码不合汿 */
...
enum MqttDataPointType {
    MQTT_DPTYPE_TRIPLE = 2,  /**< 包含数据流名称?时间戳和数据点便 */
    MQTT_DPTYPE_BINARY = 4,   /**< 包含二进制数据的数据瀿 */
    MQTT_DPTYPE_FLOAT = 7
};
...
struct MqttContext {
    ...
    int (*handle_conn_ack)(void *arg, char flags, char ret_code);
        /**< 处理连接响应的回调函数， flags取?@see MqttConnAckFlag?
             ret_code的?为 @see MqttRetCode? 成功则返回非负数
         */
    ...
    int (*handle_publish)(void *arg, uint16_t pkt_id, const char *topic,
        ...
        /**< 处理发布数据的回调函数， pkt_id为数据包的ID，topic?
             数据懿属的Topic? payload为数据的起始地址? payloadsize?
    ...
    int (*handle_pub_ack)(void *arg, uint16_t pkt_id);
        /**< 处理发布数据确认的回调，pkt_id为被确认的发布数据数据包的ID，成功则返回非负擿 */

    void *handle_pub_rec_arg; /**< 处理发布数据已接收的回调函数的关联参擿 */
    int (*handle_pub_rec)(void *arg, uint16_t pkt_id);
        /**< 处理发布数据已接收的回调函数，pkt_id为布数据已接收数据包的ID，成功则返回非负擿 */

    void *handle_pub_rel_arg; /**< 处理发布数据已释放的回调函数的关联参擿 */
    int (*handle_pub_rel)(void *arg, uint16_t pkt_id);
        /**< 处理发布数据已释放的回调函数? pkt_id为发布数据已释放数据包的ID，成功则返回非负擿 */

    void *handle_pub_comp_arg; /**< 处理发布数据已完成的回调函数的关联参擿 */
    int (*handle_pub_comp)(void *arg, uint16_t pkt_id);
        /**< 处理发布数据已完成的回调函数，pkt_id为发布数据已完成数据包的ID，成功则返回非负擿 */
    ...
    int (*handle_unsub_ack)(void *arg, uint16_t packet_id);
        /**< 处理取消订阅确认的回调函擿, pkt_id为取消订阅数据包的ID，成功则返回非负擿 */
    ...
    int (*handle_cmd)(void *arg, uint16_t pkt_id, const char *cmdid,
        ...
        /**< 处理命令的回调函擿, cmdid 为命令ID? timestamp为命令时间戳，为0标示无时间戳?
		     desc为命令描述， cmdarg为命令参数，cmdarg_len为命令参数长度，
             dup为命令是否为重发状俁， qos为QoS等级
			 成功则返回非负数
         */
};
 ...
 * @return 成功则返噿 @see MQTTERR_NOERROR
 * @remark ctx初始化成功后，不再使用时调用@see Mqtt_DestroyContext钿歿
 */
...
/**
 * 发翁数据包
 * @param buf 保存将要发翁数据包的缓冲区对象
 * @param offset 从缓冲区的offset字节处开始发辿
 * @return 成功则返回MQTTERR_NOERROR
 */
...
/**
 * 封装连接请求数据势
 * @param buf 存储数据包的缓冲区对诿
 * @param keep_alive 保活时间间隔，单位秒
 * @param id 客户端标识码
 * @param clean_session ?0时，继续使用上一次的会话，若无上次会话则创建新的会话?
 *        ?1时，删除上一次的会话，并建立新的会话
 * @param will_topic "遗言"消息发翁到的topic
 * @param will_msg "遗言"消息，当服务器发现设备下线时，自动将该消息发送到will_topic
 * @param msg_len "遗言"消息的长帿
 * @param qos Qos等级
 * @param will_retain ?0时，服务器发送will_msg后，将删除will_msg，否则将保存will_msg
 * @param user 用户县
 * @param password 密码
 ...
 * @param topic 数据发翁到哪个topic
 * @param payload 将要被发布的数据块的起始地址
 * @param size 数据块大小（字节数）
 * @param qos QoS等级
 * @param retain 雿0时，服务器将该publish消息保存到topic下，并替换已有的publish消息
 * @param own 雿0时，拷贝payload到缓冲区
 * @return 成功则返回MQTTERR_NOERROR
 * @remark 当own?0时，payload必须在buf被销毁或重置前保持有擿
 */
...
/**
 * 添加隿要订阅的Topic到已有的订阅数据包中
 * @param buf 存储订阅数据包的缓冲区对诿
 ...
 * @param buf 存储数据包的缓冲区对诿
 * @param pkt_id 数据包ID
 * @param topics 将要取消订阅的Topic，不能包县'#'吿'+'
 * @param topics_len
 * @return 成功返回MQTTERR_NOERROR
 */
int Mqtt_PackUnsubscribePkt(struct MqttBuffer *buf, uint16_t pkt_id, const char *topics[], int topics_len);
/**
 * 添加隿要取消订阅的Topic到已有的取消订阅数据包中
 * @param buf 存储取消订阅数据包的缓冲区对诿
 * @param topic 隿要取消的Topic
 * @return 成功返回MQTTERR_NOERROR
 */
 ...
 * @param ret 返回数据起始地址
 * @param ret_len 返回数据字节擿
 * @param own 雿0时，拷贝ret到缓冲区
 * @return 成功返回MQTTERR_NOERROR
 * @remark 当own?0时，ret必须确保在buf被销毁或重置前一直有擿
 */
...
/**
 * 庿始封装@see MQTT_PKT_TRIPLE类型的数据点（OneNet扩展?
 * @param buf 用于存储数据包的缓冲区对诿
 * @param pkt_id 数据包ID，非0
 * @param qos QoS等级
 * @param retain 雿0时，服务器将该publish消息保存到topic下，并替换已有的publish消息
 * @param topic 雿0时，$dp，否冿 $crsp
 * @return 成功返回MQTTERR_NOERROR
 * @remark 当完成数据点的数据添加后隿调用@see Mqtt_PackDataPointFinish?
 *         重发时，调用@see Mqtt_SetPktDup 设置数据包为重发状使
 */
int Mqtt_PackDataPointStart(struct MqttBuffer *buf, uint16_t pkt_id,
                            enum MqttQosLevel qos, int retain, int topic);
/**
 * 添加空?数据点，用于清除retain的数拿
 * @param buf 存储MQTT_DPTYPE_TRIPLE类型的数据包的缓冲区对象
 * @param dsid 数据流id
 * @return 成功返回MQTTERR_NOERROR
 * @remark 调用Mqtt_PackDataPointStart庿始封装数据点时，retain隿设置?0?
 *         然后通过添加空?数据点的方式可以清除已retain的消忿
 */
int Mqtt_AppendDPNull(struct MqttBuffer *buf, const char *dsid);
/**
 * 添加整数类型数据到@see MQTT_PKT_TRIPLE类型的数据包?
 * @param buf 存储MQTT_DPTYPE_TRIPLE类型的数据包的缓冲区对象
 * @param dsid 数据流ID
 * @param ts  格林威治时间，从1970-01-01T00:00:00.000庿始的毫秒时间戳，
 *            ?0或负数时，系统取默认时间
 * @param value 数据点的便
 * @return 成功返回MQTTERR_NOERROR
 */
...
/**
 * 添加字符串类型数据到@see MQTT_PKT_TRIPLE类型的数据包?
 * @param buf 存储MQTT_DPTYPE_TRIPLE类型的数据包的缓冲区对象
 * @param dsid 数据流ID
 * @param ts  格林威治时间，从1970-01-01T00:00:00.000庿始的毫秒时间戳，
 *            ?0或负数时，系统取默认时间
...
/**
 * 添加浮点数类型数据到@see MQTT_PKT_TRIPLE类型的数据包?
 * @param buf 存储MQTT_DPTYPE_TRIPLE类型的数据包的缓冲区对象
 * @param dsid 数据流ID
 * @param ts  格林威治时间，从1970-01-01T00:00:00.000庿始的毫秒时间戳，
 *            ?0或负数时，系统取默认时间
...
/**
 * 添加复合类型数据到@see MQTT_PKT_TRIPLE类型的数据包?
 * @param buf 存储MQTT_DPTYPE_TRIPLE类型的数据包的缓冲区对象
 * @param dsid 数据流ID
 * @param ts  格林威治时间，从1970-01-01T00:00:00.000庿始的毫秒时间戳，
 *            ?0或负数时，系统取默认时间
 ...
 * @param name 子复合类型的名字
 * @return 成功返回MQTTERR_NOERROR
 * @remark 完成子复合类型数据的添加后，隿调用 @see  Mqtt_AppendDPFinishSubobject
 */
...
/**
 * 结束封装数据瀿
 * @param buf 存储@see MqttDataPointType类型数据包的缓冲区对诿
 * @return 成功返回MQTTERR_NOERROR
 */
 ...
 * @param dsid 数据流ID
 * @param desc 数据点的描述信息
 * @param time 格林威治时间，从1970-01-01T00:00:00.000庿始的毫秒时间戳，
 *             ?0或负数时，系统取默认时间
 * @param bin 二进制数据的起始地址
 * @param size 二进制数据的字节擿
 * @param qos QoS等级
 * @param retain 雿0时，服务器将该publish消息保存到topic下，并替换已有的publish消息
 * @param own 雿0时，拷贝bin到缓冲区
 * @param save 雿0时，服务器将存储数据，否则不嫿
 * @return 成功返回MQTTERR_NOERROR
 * @remark 当own?0时，bin必须在buf被销毁或重置前保持有擿
 */
